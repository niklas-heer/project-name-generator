...
import { listModels, getDefaultModel, getModelDetails } from "./openrouter";
...
async function main() {
  ...
  const modelsStr = modelsList.join(", ");
  const defaultGenerateModel = await getDefaultModel("generate");
  const defaultJudgeModel = await getDefaultModel("judge");
    ...
    .action(async (description: string, options: GenerateCommandOptions) => {
      ...
      if (options.untilAvailable) {
        ...
        if (isNaN(targetAvailable) || targetAvailable < 1) {
          console.error(
            chalk.red("--until-available must be a positive number"),
          );
          process.exit(1);
        }
  ...
  program
    ...
    .action(() => {
      console.log(chalk.bold("\nAvailable models:\n"));
      for (const [alias, config] of Object.entries(modelDetails)) {
        const isDefaultGen = alias === defaultGenerateModel;
        const isDefaultJudge = alias === defaultJudgeModel;
        let badge = "";
        if (isDefaultGen) badge = chalk.green(" (default: generate)");
        if (isDefaultJudge) badge = chalk.blue(" (default: judge)");
        if (isDefaultGen && isDefaultJudge)
          badge = chalk.magenta(" (default: both)");
        console.log(`  ${chalk.bold(alias)}${badge}`);
        console.log(`    ${chalk.dim(config.description)}`);
        console.log(`    ${chalk.dim(config.id)}`);
      }
      console.log();
    });
